...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vabsen`  AS SELECT `a`.`id_absen` AS `id_absen`, `s`.`nama` AS `nama_siswa_a`, `vj`.`nama_mapel_j` AS `nama_mapel_a`, `a`.`waktu` AS `waktu`, `a`.`tanggal` AS `tanggal`, CASE `a`.`keterangan` WHEN 'H' THEN 'Hadir' WHEN 'S' THEN 'Sakit' WHEN 'I' THEN 'Izin' WHEN 'A' THEN 'Alfa' WHEN 'T' THEN 'Terlambat' ELSE `a`.`keterangan` END AS `keterangan_a` FROM ((`absen` `a` join `siswa` `s` on(`a`.`id_siswa` = `s`.`id_siswa`)) join `vjadwal` `vj` on(`a`.`id_jadwal` = `vj`.`id_jadwal`)) ;
